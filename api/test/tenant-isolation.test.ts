...
describe("tenant isolation", () => {
  it("user cannot list another user's batches", async () => {
    ...
    const { json } = await fetchJson("/api/v1/batches", { headers: await authHeaders(ALICE) });
  ...
  it("user cannot read another user's batch", async () => {
    ...
    const { status } = await fetchJson(`/api/v1/batches/${bobBatch}`, { headers: await authHeaders(ALICE) });
  ...
  it("user cannot delete another user's batch", async () => {
    ...
    await fetchJson(`/api/v1/batches/${bobBatch}/abandon`, { method: "POST", headers: await authHeaders(BOB) });
    const { status } = await fetchJson(`/api/v1/batches/${bobBatch}`, {
      method: "DELETE",
      headers: await authHeaders(ALICE),
    });
  ...
  it("user cannot see another user's devices", async () => {
    ...
    const { json } = await fetchJson("/api/v1/devices", { headers: await authHeaders(BOB) });
  ...
  it("dashboard only shows user's own data", async () => {
    ...
    const { json } = await fetchJson("/api/v1/dashboard", { headers: await authHeaders(ALICE) });
  ...
  it("claiming a device gives ownership of its readings", async () => {
    ...
    const { json: devices } = await fetchJson("/api/v1/devices", { headers: await authHeaders(ALICE) });
    expect(devices.items.find((d: any) => d.id === "orphan-pill")).toBeDefined();

    // Bob cannot
    const { json: bobDevices } = await fetchJson("/api/v1/devices", { headers: await authHeaders(BOB) });
